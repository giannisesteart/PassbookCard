 ...
 */
    ...
    if (! isset ($_POST['serialnumber'] ) || !isset ($_POST['hotelcode']) ) 
	{
	...
	$passbookdata = $passbooktable->passbookdata ($hotelcode, $serialnumber);
	$passbookstatus= $passbookdata['status'] ;
	$passtype =$passbookdata['passtype'] ;
	...
	foreach ($guests as $guestcodeactual) {
		
		
	 /*
	  *  If card is AH3 set the guestcode as registartioncode
	  */
	 $registrationpair = $registerstable->isregisteredAH($hotelcode, $guestcodeactual, $serialnumber, NULL);
	 if ($registrationpair==NULL) { echo " Cannot send for ".$guestcode." Please retry ";
	                                continue; 
	 } else {
		 //if AH3 set the registration guestcode else the actual guestcode
		$guestcode=  $registrationpair['registrationguest'] ; 
	 }
	...
	// IF is not registered
     if ($registrationpair['registrationid'] ==NULL) {
		 ...
		 if (sendemail ($guestcode, $serialnumber,$hotelcode))
           $registerstable->insert_registration ($guestcode, $hotelcode, $serialnumber, $passbookstatus, actualtime ());
	     else  echo " Unable to send email for guest:".$guestcode ;
		...
		if (($registrationpair['deviceid'] !=NULL) && ($email="No"))
		 { 
			$ret= sendPush ($guestcode, $registrationpair['deviceid'], $passtype );
			
		} else 
		  {  
	        
	        $ret=sendemail ($guestcode, $serialnumber,$hotelcode);
	      }
		  if ($ret) $registerstable->update_registration ( $guestcode, $serialnumber, actualtime ());
		  else  echo " Unable to send push or email for guest:".$guestcode ;   
          break;
