     ...
     require_once ($MYROOT."/PHPUtil/config.php");
	   ...
	   if( ! $rec  ) { //Reward system not existent
		 ...
		 $sql = "SELECT 
                    ...
                    sum (MaxQuantity) as MaxQuantity,
	                sum (EPointsFB)as  EPointsFB,
                    sum (EPointsMobCheck) as EPointsMobCheck,
                    sum(EPointsStay) as EPointsStay,
                    sum (EPointsRoomService) as EPointsRoomService,
                    sum(EPointsShopping) as EPointsShopping,
                    sum(EPointsFirstSign) as EPointsFirstSign,
                    sum(EPointsLogin) as EPointsLogin,
		            sum (EPointsMobBooking) as EPointsMobBooking,
		            sum(EPointsHotel) as EPointsHotel,
					sum(Stays) as Stays,
				  ...
				  ON   a.email = b.email) c 
					INNER JOIN RATES ON c.code=RATES.guestcode  
					  WHERE RewardsOn=1 ; "; 
	    ...
	    $RewardsOn =1;
		 ...
		 $Stays =  $rec['Stays'];
		 $Nights = $rec['Nights'];
		 if   (($Points + $EPointsFB + $EPMobCheck + $EPointsStay +$EPointsRoomService
                    		 +$EPointsShopping +$EPointsFirstSign  +$EPointsLogin+$EPointsMobBooking + ($EPointsHotel)) ==NULL )
		{
			 echo "No Rates yet. Cant send the passbook.\n";
			 die () ;
		}								 
		...
		if ($RewardsOn ==1) {
			$Memberpoints = " "	. ($Points + $EPointsFB + $EPMobCheck + $EPointsStay +$EPointsRoomService
                    		 +$EPointsShopping +$EPointsFirstSign  +$EPointsLogin+$EPointsMobBooking + ($EPointsHotel)) ;
