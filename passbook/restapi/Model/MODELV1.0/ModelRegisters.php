...
class ModelRegisters
{
	...
	//insert registration
	public function insert_registration (   $guestcode, $serialnumber, $status)
	{  
      $sql = "INSERT INTO REGISTRATIONS ( guestcode,  serialnumber, status) 
              VALUES ( ".$guestcode.",'".$serialnumber."','".$status. "');" ;
	...
	{  
	   ...
	   ($sincedate!=NULL)?
		...
		:
		$sql = "SELECT * FROM REGISTRATIONS WHERE status ='". UPDATE_CARD. "' AND guestcode=" . $guestcode." AND deviceid=".$deviceid.";";
	  ...
	  $stmt = sqlsrv_query($this->connms,$sql);
       if( $stmt === false)
        ...
        while  ( $rec = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
		    array_push($serials,$rec['serialnumber'] );    
		} 
	...
	public function isregistered_device( $guestcode, $serialnumber)
	{  
	  $sql = "SELECT * FROM REGISTRATIONS WHERE status ='". UPDATE_CARD. "' AND guestcode=" . $guestcode." AND serialnumber='".$serialnumber."';"  ;
        $stmt = sqlsrv_query($this->connms,$sql);
        if( $stmt === false)
