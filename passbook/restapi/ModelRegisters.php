<?php namespace Model\Registers;

 require_once 'Defines.php';
 
/* Class realizes Restapi calls TABLE REGISTRATIONS
...
class ModelRegisters
{
	...
	{  
      $sql = "UPDATE REGISTRATIONS SET status ='". DELETED_FROM_USER. "' WHERE guestcode=" . $guestcode. " AND deviceid=".$deviceid. 
	...
	{  
	    ...
	    $sql = "SELECT * FROM REGISTRATIONS WHERE status ='". UPDATE_CARD. "' AND guestcode=" . $guestcode.
		               " AND serialnumber='".$serialnumber."' AND last_updated > '".$sincedate."';"  ;
	...
	{  
	   ...
	   ($sincedate!=NULL)?
	    $sql = "SELECT guestcode, serialnumber FROM REGISTRATIONS WHERE status ='". UPDATE_CARD. "' AND guestcode=" . $guestcode.
		                   " AND deviceid=".$deviceid. " AND last_updated >'".$sincedate."';"  
		:
		$sql = "SELECT guestcode, serialnumber FROM REGISTRATIONS WHERE status ='". UPDATE_CARD. "' AND guestcode=" . $guestcode." AND deviceid=".$deviceid.";";
    
	  $stmt = sqlsrv_query($this->connms,$sql);
       if( $stmt === false)
         {  $this->log->error ("Error in query preparation/execution LOOKUPSERIALS :>RESTAPI".$guestcode." &deviceid=".$deviceid); 
            	 
            return ( array ('error'=>TRUE, 'serialnumbers'=> $serials) );
        } 	 
		...
		return array ( 'error'=>FALSE, 'serialnumbers'=> $serials); 
	}
	   ...
	   if ($status!=NULL)
        ...
        if( $stmt === false)
         {  $this->log->error ("Error in query preparation/execution SELECT :>RESTAPI".$guestcode.$serialnumber);
            ...
            return (array ('registrationid'=>NULL, 'deviceid'=>NULL, 'last_updated'=>NULL, "status"=>null));
         } 	 
		 
        if ( $rec = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
		    return  (array ('registrationid'=>$rec['id'],'deviceid'=>$rec['deviceid'],'last_updated'=>$rec['last_updated'] ,"status"=>$rec['status'])) ; 
		}else {	
	...
	{  
	 ...
	 if ($serialnumber="AH3") 
	   $sql = "SELECT * FROM
        ...
        if ( $rec = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
		    return  (array ('registrationguest'=>$rec['guestcode'], 'registrationid'=>$rec['id'],'deviceid'=>$rec['deviceid'],'last_updated'=>$rec['last_updated'] ,"status"=>$rec['status'])) ; 
		}else {	
		  //$this->log->error ("Registration for ". $guestcode." doesnt exists:>RESTAPI");
          return (array ('registrationguest'=>$guestcode,'registrationid'=>NULL, 'deviceid'=>NULL, 'last_updated'=>NULL, "status"=>null)); 
		}
	}
